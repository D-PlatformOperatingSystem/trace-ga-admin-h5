<template>
	<view class="body">
		...
		<view class="tab_numb">
			...
			<view class="tab-list">
				<view class="tab-item" @click="toBatchEdit(item.id)" v-for="(item,index) in SgsList" :key="index">
					<view class="tab-top">
					...
					<view class="tab-content">
						...
						<view class="tab-li">
							<text>{{$t('SGS.AutStatus')}}：</text>
							<text class="hui-text" v-if="item.sgsStatus === 1">{{item.sgsStatus_dictText}}</text>
							<text class="green-text" v-else-if="item.sgsStatus === 2">{{item.sgsStatus_dictText}}</text>
							<text class="red-text" v-else>{{item.sgsStatus_dictText}}</text>
						</view>
						<view class="tab-li">
							<text>{{$t('SGS.cerDate')}}：</text>
							<text>{{item.issueTime}}</text>
						</view>
						<view class="tab-li">
							<text>{{$t('SGS.ExpTime')}}：</text>
							<text>{{item.issueExpiration}}</text>
						</view>
					</view>
				</view>
				...
				<view class="pop-radio">
					<radio-group class="block" @change="RadioChange">
						<view class="cu-form-group">
							...
							<radio class='radio' :class="radio=='issueTime'?'checked':''"
								:checked="radio=='issueTime'?true:false" value="issueTime"></radio>
						</view>
						<view class="cu-form-group">
							<view class="title">
								{{$t('SGS.ExpTime')}}
							</view>
							<radio class='radio' :class="radio=='issueExpiration'?'checked':''"
								:checked="radio=='issueExpiration'?true:false" value="issueExpiration"></radio>
						</view>
					</radio-group>
				</view>
			</view>
			...
			<view class="popup-bottom">
				...
				<view class="popup-padd">
					<view class="cu-form-group cu-form-group-bg">
						<picker mode="date" :value="issueTimeStart" @change="DateChange">
							<view class="picker">
								{{issueTimeStart}}
							</view>
						</picker>
					...
					<view class="cu-form-group cu-form-group-bg">
						<picker mode="date" :value="issueTimeEnd" @change="DateEndChange">
							<view class="picker">
								{{issueTimeEnd}}
							</view>
						</picker>
				...
				<view class="popup-padd">
					<view class="cu-form-group cu-form-group-bg">
						<picker mode="date" :value="issueExpirationStart" @change="GetIssueStart">
							<view class="picker">
								{{issueExpirationStart}}
							</view>
						</picker>
					</view>
					<view class="cu-zhi">
					...
					<view class="cu-form-group cu-form-group-bg">
						<picker mode="date" :value="issueExpirationEnd" @change="GetIssueEnd">
							<view class="picker">
								{{issueExpirationEnd}}
							</view>
						</picker>
	...
	export default {
		data() {
			return {
				...
				pickerarea: -1,
				massifList: [],
				issueTimeStart: '2021-05-30',
				issueTimeEnd: '2021-06-01',
				issueExpirationStart: '2021-05-30',
				issueExpirationEnd: '2021-06-01',
				SgsList: [],
				page: 1,
				pageSize: 10,
				pageTotal: 0,
		...
		methods: {
			...
			getDataList() {
				...
				that.$api.getSgsList({
					...
					console.log(res);
					if (res.data.code === 200) {
						that.$data.SgsList = res.data.result.records;
						that.page = that.$data.SgsList.length == that.pageSize ? ++that.page : that.page;
					} else {
						uni.showToast({
							title: res.data.message,
							icon: 'success',
			...
			DateChange(e) {
				this.issueTimeStart = e.detail.value;
			},
			DateEndChange(e) {
				this.issueTimeEnd = e.detail.value;
			},
			GetIssueStart(e) {
				this.issueExpirationStart = e.detail.value;
			},
			GetIssueEnd(e) {
				this.issueExpirationEnd = e.detail.value;
			},
			PickerChange(e) {
			...
			batchSort() {
				...
				if (that.$data.indexInfor === -1 &&
					...
					that.$data.issueTimeStart === '2021-05-30' &&
					that.$data.issueTimeEnd === '2021-06-01' &&
					that.$data.issueExpirationStart === '2021-05-30' &&
					that.$data.issueExpirationEnd === '2021-06-01'
				) {
					data = {
			...
			batchScreen() {
				...
				let data = {
					...
					superQueryParams: JSON.stringify([{
							rule: "ge",
							type: "date",
							val: that.$data.issueTimeStart,
							field: "issueTime"
						},
						{
							rule: "le",
							type: "date",
							val: that.$data.issueTimeEnd,
							field: "issueTime"
						},
						{
							rule: "ge",
							type: "date",
							val: that.$data.issueExpirationStart,
							field: "issueExpiration"
						},
						{
							rule: "le",
							type: "date",
							val: that.$data.issueExpirationEnd,
							field: "issueExpiration"
						}
					])
				}
				...
				that.$api.getSgsList({
					...
					if (res.data.code === 200) {
						...
						if (that.$data.pageTotal > 10) {
							that.isNoMoreData = that.$data.SgsList.length == that.pageSize ? false :
							true;
							that.page = that.$data.SgsList.length == that.pageSize ? ++that.page : that
							.page; 
							that.$data.SgsList = that.$data.SgsList.concat(res.data.result.records);
						}
					} else {
						uni.showToast({
							title: res.data.message,
...
<style scoped>
	...
	.tab-li text.red-text {
		color: #DE3939;
	}

	.tab-li text.hui-text {
		color: #9B9DAD;
	}
